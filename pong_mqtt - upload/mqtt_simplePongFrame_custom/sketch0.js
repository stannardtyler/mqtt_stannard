// Pong variant where you hit the ball with your paddle to cover the space with your color.
...
function startandChooseServerOrClient() {
  ...
  label1.style('font-size', '25px');
  label1.style('text-align', 'center');
  label1.position(width/2 - 50,220);
  label2 = createP("Choose either of Judge Bot or Player");
  label2.size(300,30);
  label2.style('color', '#FFF');
  label2.style('font-size', '18px');
  label2.position(100 + width/2-130,275);
...
function AfterStartServer() {
  ...
  label2.html(" ");
  label2.position(100 + width/2-80,200);
  label2.size(300,30);
  label2.html("This is the Server View");
  label2.position(100 + width/2-80,200);
  label2.size(300,30);
  resultlabel = createP();
...
function AfterStartClient() {
  ...
  label2.html("Move your paddle around, then  hit the  ball");
  label2.position(100 + width/2-150,180);
  label2.size(450,30);
  submitBtn1.style('visibility', 'hidden');
  submitBtn2.style('visibility', 'hidden');
  resultlabel = createP();
...
background(bgr, 50, bgb);
  ...
  if (playState) {
    if (myClientName != "mserver") {  // The players loop
      ...
      stroke(0,155,30);
      fill(180);
      ellipse(play1X, play1Y,40,40);
      stroke(0,30,155);
      fill(180);
      ellipse(play2X, play2Y,40,40);
       ...
       if(dist(ballX, ballY, play1X, play1Y) < 10) {
           speedX += random(-2,2);
           speedY += random(-2,2);
           speedX = -speedX;
           speedY = -speedY;
       ...
       if(dist(ballX, ballY, play2X, play2Y) < 10) {
           speedX += random(-2,2);
           speedY += random(-2,2);
           speedX = -speedX;
           speedY = -speedY;
